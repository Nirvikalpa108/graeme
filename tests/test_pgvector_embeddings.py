...
def test_embedding_table_exists(db_cursor):
    cur = db_cursor 
    cur.execute("""
        SELECT EXISTS (
...
def test_all_products_embedded(db_cursor):
    cur = db_cursor 
...
def test_no_null_embeddings(db_cursor):
    cur = db_cursor 
...
def test_embedding_vector_dimensions(db_cursor):
    cur = db_cursor 
...
def test_generate_embedding_from_db_description(db_cursor):
    cur = db_cursor 
...
def test_cosine_similarity_of_stored_embedding(db_cursor):
    cur = db_cursor 
